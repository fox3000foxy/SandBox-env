...
export function detectBackend(preference: SandboxBackend): {
  ...
  const candidates: Array<"bwrap" | "firejail"> =
    preference === "auto"
      ? ["bwrap", "firejail"]
      : preference === "bwrap"
      ? ["bwrap"]
  ...
  throw new Error(
    `No sandbox backend found (tried: ${tried}).\n` +
      "Install one with:\n" +
...
export function checkTorProxy(opts: Required<TorOptions>): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    socket.on("timeout", () => {
      ...
      reject(
        new Error(
          `Tor SOCKS5 proxy unreachable at ${opts.host}:${opts.port}.\n` +
    ...
    socket.on("error", (err) => {
      reject(
        new Error(
          `Cannot connect to Tor SOCKS5 at ${opts.host}:${opts.port}: ${err.message}`
        )
      );
    });
  });
